...
def main():
    student_id = 151211  # Tutaj należy wpisać swój numer indeksu

    data = BytesIO()
    with ZipFile(data, 'w', ZIP_DEFLATED) as zip_file:
        base_path = Path.cwd()
        for file in base_path.rglob('*'):
            if (file.name in ('data.yaml', 'test.py', 'train.py', 'bestEnum200.pt', 'yolov8n.pt', 'yolov8m.pt', 'yolov8x.pt')
                    or file.is_relative_to(base_path / 'env')
                    or file.is_relative_to(base_path / 'runs')
                    or file.is_relative_to(base_path / '.idea')
                    or file.is_relative_to(base_path / '.imgs')
